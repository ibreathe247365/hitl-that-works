...
// Helper functions
export const verifyWebhookSignature = (request: NextRequest, body: string): boolean => {
...
export const createErrorResponse = (message: string, error: string, status: number, details?: any) => 
  NextResponse.json({ message, error, ...(details && { details }) }, { status });

export const getThreadFromPayload = async (payload: WebhookPayload): Promise<Thread> => {
  ...
  if (stateId) {
    ...
    if (!thread) {
      throw new Error(`Could not find thread state for ${stateId}`);
    }
    return thread;
  }
