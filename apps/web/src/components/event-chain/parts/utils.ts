...
export function buildOperationGraph(events: Event[]) {
    ...
    return { eventIndex, operationIdToChildren, rootOperationEvents, nonOperationEvents } as const;
}
...
export function getEventLabel(currentEvent: Event) {
    switch (currentEvent.type) {
        ...
        case "tool_call":
        case "function_call":
            return (currentEvent.data as any).name || (currentEvent.data as any).function_name || "Tool Call";
        case "ai_response":
...
export function getEventStatusBlinkClass(currentEvent: Event) {
    ...
    return colorClass === "bg-gray-500" || colorClass === "bg-muted-foreground/40" ? "" : "animate-pulse";
}
