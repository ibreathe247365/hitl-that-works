...
import { getEventLabel, getEventStatusBlinkClass, getEventStatusColor } from "./utils";
...
import { WebhookOperationContent, WebhookReceivedContent } from "./events/WebhookEvents";
...
export function EventContent({ event }: { event: Event }): ReactElement {
    switch (event.type) {
        ...
        default:
            return (
                <pre className="overflow-x-auto rounded-lg border bg-muted p-3 text-xs">{JSON.stringify(event.data, null, 2)}</pre>
            );
    }
}
        ...
        <Task defaultOpen={false}>
            <TaskTrigger title={label}>
                <div className="flex items-center gap-2">
                    <span className={`inline-block size-2 rounded-full ${getEventStatusColor(event)} ${getEventStatusBlinkClass(event)}`} />
                    <Badge variant="outline" className="text-xs">{label}</Badge>
                    <span className="text-muted-foreground text-xs">
                        {new Date(((event.data as any)?.timestamp || Date.now()) as number).toLocaleTimeString()}
                    </span>
                </div>
            </TaskTrigger>
            <TaskContent>
