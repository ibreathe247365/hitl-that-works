...
export function AIStepContent({ event }: { event: Event }): ReactElement | null {
    ...
    if (payload.intent === "nothing_to_do") {
        return (
            <div className="space-y-2">
                ...
                <p className="text-muted-foreground text-sm">{payload.message ?? "No action required."}</p>
            </div>
        );
    }
        ...
        return (
            <div className="space-y-2">
                <div className="flex items-center gap-2">
                    ...
                    <h4 className="text-sm font-medium">AI Decision</h4>
                    <Badge variant="secondary">Task Complete</Badge>
                </div>
                <p className="text-muted-foreground text-sm">{payload.message ?? "Done for now."}</p>
            </div>
        );
    }
...
export function AIStepStep({ event }: { event: Event }): ReactElement {
    return (
        <ChainOfThoughtStep label="AI Step" description="Reasoning step" status="complete">
