...
export function HumanResponseContent({ event }: { event: Event }): ReactElement {
    const response = (event.data as any)?.data ?? (event.data as any)?.message ?? (event.data as any)?.text;
...
export function HumanResponseStep({ event }: { event: Event }): ReactElement {
    return (
        <ChainOfThoughtStep label="Human Response" description="User provided additional information" status="complete">
