...
export function CalculateResultContent({ event }: { event: Event }): ReactElement {
                ...
                <div>
                    ...
                    <ul className="list-disc pl-5">
                        {data.steps.map((s: any, i: number) => (
                            <li key={i} className="font-mono text-xs">{String(s)}</li>
                        ))}
                    </ul>
                </div>
            )}
...
export function CalculateResultStep({ event }: { event: Event }): ReactElement {
    return (
        <ChainOfThoughtStep label="Result" description="Calculation Result" status="complete">
