...
import { evaluateExpression, validateMathematicalExpression } from './tools/calculator'
...
const functionHandlers: Record<string, (thread: Thread, kwargs: FunctionKwargs) => Promise<Thread>> = {
...
const handleHumanContactCompleted = async (thread: Thread, payload: HumanContactCompleted): Promise<void> => {
  thread.events.push({
    type: 'human_response',
...
const handleFunctionCallCompleted = async (thread: Thread, payload: FunctionCallCompleted): Promise<void> => {
  // Handle rejection
...
export const handleHumanResponse = async (
  ...
  switch (payload.type) {
    case 'human_contact.completed':
      return await handleHumanContactCompleted(thread, payload as HumanContactCompleted);
    case 'function_call.completed':
      return await handleFunctionCallCompleted(thread, payload as FunctionCallCompleted);
    default:
