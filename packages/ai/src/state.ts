...
export async function getThreadStateWithMetadata(stateId: string): Promise<ThreadStateWithMetadata | null> {
  const state = await redis.get<string>(stateId)
  if (!state) return null;
  ...
  try {
    ...
    console.error(`Failed to parse thread state with metadata ${stateId}:`, error);
    return null;
  }
}
