 ...
 *
 * **Return Type:**
 * - **Non‑streaming:** types.ClarificationRequest | types.DoneForNow | types.IntentCalculate | types.NothingToDo | types.Await
 * - **Streaming Partial:** ClarificationRequest | DoneForNow | IntentCalculate | NothingToDo | Await
 * - **Streaming Final:** types.ClarificationRequest | types.DoneForNow | types.IntentCalculate | types.NothingToDo | types.Await
 *
 * **Usage Patterns:**
...
export function useDetermineNextStep(
  ...
  let action: ServerAction = Actions.DetermineNextStep;
  if (isStreamingProps(props)) {
    action = StreamingActions.DetermineNextStep;
    return useBamlAction(action, props)
  } else {
    return useBamlAction(action, props as HookInput<'DetermineNextStep', { stream: false }>)
  }
}
/**
 * A specialized hook for the PerformCalculation BAML function that supports both streaming and non‑streaming responses.
 *
 * **Input Types:**
 *
 * - expression: string
 ...
 *
 * **Return Type:**
 * - **Non‑streaming:** types.Calculation
 * - **Streaming Partial:** Calculation
 * - **Streaming Final:** types.Calculation
 *
 * **Usage Patterns:**
 * 1. **Non‑streaming (Default)**
 *    - Best for quick responses and simple UI updates.
 ...
 * ```tsx
 * // Basic non‑streaming usage:
 * const { data, error, isLoading, mutate } = usePerformCalculation({ stream: false});
 *
 * // Streaming usage:
 ...
 * ```
 */
export function usePerformCalculation(props: HookInput<'PerformCalculation', { stream: false }>): HookOutput<'PerformCalculation', { stream: false }>
export function usePerformCalculation(props?: HookInput<'PerformCalculation', { stream?: true }>): HookOutput<'PerformCalculation', { stream: true }>
export function usePerformCalculation(
  props: HookInput<'PerformCalculation', { stream?: boolean }> = {},
): HookOutput<'PerformCalculation', { stream: true }> | HookOutput<'PerformCalculation', { stream: false }> {
  let action: ServerAction = Actions.PerformCalculation;
  if (isStreamingProps(props)) {
    action = StreamingActions.PerformCalculation;
    return useBamlAction(action, props)
  } else {
    return useBamlAction(action, props as HookInput<'PerformCalculation', { stream: false }>)
  }
}
/**
 * A specialized hook for the SquashResponseContext BAML function that supports both streaming and non‑streaming responses.
 *
 * **Input Types:**
 *
 * - thread: string
 ...
 * ```tsx
 * // Basic non‑streaming usage:
 * const { data, error, isLoading, mutate } = useSquashResponseContext({ stream: false});
 *
 * // Streaming usage:
 ...
 * ```
 */
export function useSquashResponseContext(props: HookInput<'SquashResponseContext', { stream: false }>): HookOutput<'SquashResponseContext', { stream: false }>
export function useSquashResponseContext(props?: HookInput<'SquashResponseContext', { stream?: true }>): HookOutput<'SquashResponseContext', { stream: true }>
export function useSquashResponseContext(
  props: HookInput<'SquashResponseContext', { stream?: boolean }> = {},
): HookOutput<'SquashResponseContext', { stream: true }> | HookOutput<'SquashResponseContext', { stream: false }> {
  let action: ServerAction = Actions.SquashResponseContext;
  if (isStreamingProps(props)) {
    action = StreamingActions.SquashResponseContext;
    return useBamlAction(action, props)
  } else {
    return useBamlAction(action, props as HookInput<'SquashResponseContext', { stream: false }>)
  }
}